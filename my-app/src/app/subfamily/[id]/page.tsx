...
export default function SubfamilyPage({
  ...
  const [subfamilyName, setSubfamilyName] = useState<string>('');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch(`/api/subfamily/${resolvedParams.id}`);
        if (!response.ok) {
          throw new Error('Failed to fetch subfamily languages');
        }
        const data = await response.json();
        setLanguages(data);
        if (data.length > 0) {
          setSubfamilyName(data[0].subfamily_name);
        }
      } catch (err) {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-blue-300/70 mt-2 text-lg">Language Subfamily</p>
          </div>
          <Link
            href="/"
